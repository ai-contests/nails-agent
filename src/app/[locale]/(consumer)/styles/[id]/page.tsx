...
export default function StyleDetailPage({ params }: StyleDetailProps) {
  ...
  if (!style) {
    return (
      <div className="max-w-7xl mx-auto px-8 py-24 text-center">
        <h2 className="text-xl font-bold text-ink mb-4">{t('notFound')}</h2>
        <p className="text-ink-second mb-8">{t('notFoundDesc')}</p>
        <Link href="/gallery">
          <Button variant="default">{t('returnCatalog')}</Button>
        </Link>
      </div>
    );
  }
  ...
  const resolvedImgUrl = resolveImageUrl(style.image_url);

  // Formatting title
  let title = style.style_id;
  let rawColors: string[] = ['Rose Quartz', 'Lilac'];
  try {
  ...
  const getTranslatedColor = (color: string) => {
    ...
    return val === key ? color : val;
  };

  const colorFamily = rawColors.map(c => getTranslatedColor(c)).join(' | ');

  let lengthKey = 'medium';
  try {
    const lengths = JSON.parse(style.length_tags);
    if (lengths.length > 0) {
      lengthKey = lengths[0].toLowerCase();
    }
  } catch (e) {}

  const lengthTag = tCategory(lengthKey as any) || lengthKey;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24">
        ...
        <div className="relative aspect-[3/4] rounded-[24px] overflow-hidden bg-surface-warm shadow-xl">
          ...
          <div className="absolute bottom-6 left-6 bg-white/90 backdrop-blur-sm px-4 py-2 rounded-pill text-xs font-semibold text-ink shadow-md">
            {t('colorFamily')}: <span className="font-normal text-ink-second">{colorFamily}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col justify-center">
          <div className="flex gap-2 mb-4">
            ...
            <span className="bg-blush-light text-primary px-3 py-1 text-[10px] rounded-pill font-semibold">
              {t('lengthLabel', { length: lengthTag })}
            </span>
          </div>
          
          <h1 className="text-display font-bold text-ink mb-4">{title}</h1>
          <p className="text-ink-second leading-relaxed mb-8">
            {t('descParagraph', { length: lengthTag.toLowerCase(), colors: colorFamily.toLowerCase() })}
          </p>

          {/* Shades */}
          {(primaryRgb || secondaryRgb) && (
            <div className="mb-8">
              <h4 className="text-[10px] font-bold text-ink-light tracking-widest uppercase mb-3">{t('recommendedShades')}</h4>
          ...
          <div className="flex gap-4 mb-8">
            ...
            >
              <Search className="w-5 h-5 mr-2" /> {t('tryOnNow')}
            </Button>
            <Button 
              ...
              disabled={!sessionId}
              title={!sessionId ? t('tooltipScanFirst') : t('tooltipAddFavorite')}
            >
          ...
          <div className="bg-surface-warm rounded-xl p-6 flex items-center justify-between border border-c-border">
            <div>
              <div className="text-[10px] font-bold text-ink-light tracking-widest uppercase mb-1">{t('calibration')}</div>
              <div className="font-semibold text-ink">{handProfile ? t('activeProfile') : t('defaultAlignment')}</div>
            </div>
            <div className="text-right">
              <div className="text-[10px] font-bold text-ink-light tracking-widest uppercase mb-1">{t('matchScore')}</div>
              <div className="font-semibold text-success">{matchScore} {t('matchSuffix')}</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-ink">
              {handProfile ? t('similarHandsTitle') : t('trendingTitle')}
            </h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {similarStyles.map(sim => {
              const simImgUrl = resolveImageUrl(sim.image_url);
              ...
              try {
                ...
                if (colors.length > 0) {
                  ...
                  simTitle = colors.map((c: string) => getTranslatedColor(c)).join(' & ');
                }
              } catch (e) {}
