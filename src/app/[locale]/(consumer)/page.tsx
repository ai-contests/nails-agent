  ...
  return (
    <div className="flex flex-col gap-24 py-16">
      ...
      <section className="max-w-7xl mx-auto px-8 w-full grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        <div className="max-w-lg">
          ...
          <div className="flex gap-4">
            <Link href="/hand">
              <Button variant="default">{t('heroActionStart')}</Button>
            </Link>
            <Link href="/gallery">
              <Button variant="outline" className="rounded-full bg-white">{t('heroActionBrowse')}</Button>
            </Link>
          </div>
        </div>
      ...
      <section className="bg-surface-warm py-24 px-8 text-center border-y border-c-border">
        <h2 className="text-h1 font-bold text-ink mb-4">{t('magicTitle')}</h2>
        ...
        <div className="max-w-md mx-auto aspect-[3/4] bg-white rounded-card shadow-soft-glow overflow-hidden relative border border-c-border select-none">
          ...
          <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-sm text-white text-[10px] px-2.5 py-1 rounded">{t('magicBefore')}</div>
          <div className="absolute bottom-4 right-4 bg-black/60 backdrop-blur-sm text-white text-[10px] px-2.5 py-1 rounded">{t('magicAfter')}</div>
      ...
      <section className="max-w-7xl mx-auto px-8 w-full">
        <div className="flex justify-between items-end mb-8">
          <div>
            <h2 className="text-h1 font-bold text-ink mb-2">{t('trendingTitle')}</h2>
            <p className="text-ink-second text-sm">{t('trendingDesc')}</p>
          </div>
      ...
      <section className="max-w-5xl mx-auto w-full px-8">
        <div className="bg-primary rounded-[24px] p-12 text-center text-white shadow-2xl relative overflow-hidden">
          <div className="relative z-10">
            <h2 className="text-h1 font-bold mb-4">{t('transformTitle')}</h2>
            <p className="mb-8 opacity-90">{t('transformDesc')}</p>
            <Link href="/hand">
              <Button variant="secondary" size="lg" className="rounded-full px-8">{t('transformAction')}</Button>
            </Link>
          </div>
        </div>
      </section>
