...
候选池上架 / 回退候选池时也使用同一套快照重建机制：

```text
读取当前 active global_main snapshot
-> 复制当前 recommendation_items 排序列表
-> 移除回退候选池的 style_id
-> 将新上架 style_id 插入第 11 位开始
-> 多个新上架款依次插入 11、12、13...
-> 对完整列表重新编号 rank_no = 1...N
-> 写入新的 recommendation_snapshot 和完整 recommendation_items
-> 新 snapshot 标记 active，旧 snapshot 标记 archived
```

因此程序可以自动完成“原位继承”和“自动补位”：例如移除原 rank 40 的款式后，后续款式会在新快照中自动前移并重新编号。
...
工具详细参数 schema 以 `internal_data_flow_confirmed.md` 中的 `Agent 工具 Schema V1` 为准。
...
| `finding_type` | enum | `opportunity` / `anomaly` / `tag_trend` / `candidate_match` / `watch` |
| `target_type` | string | `style` / `tag` / `tag_combo` / `candidate` / `global` / `system` |
...
- 运营机会发现、异常诊断、继续观察这类“没有改数据”的结果写入 `agent_findings`。
...
Agent 证据关联表。它的作用是把 Agent 的发现、决策或复盘记忆关联回具体证据来源，方便 B 端 Chat 解释“为什么这么判断”。

这张表不代表业务动作，也不直接修改款式或推荐页。它只保存证据引用。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `evidence_link_id` | string PK | 证据关联 ID |
| `owner_type` | string | `finding` / `decision` / `memory` / `pending_review` |
| `owner_id` | string | 所属记录 ID，如 `finding_id`、`decision_id`、`memory_id` |
...
```text
运营机会发现可以引用：
- style_heat: STYLE018 最近 N 轮热度快照
- tag_heat: 裸粉短甲 tag 组合增长
...
agent_findings 1 -- N agent_evidence_links
...
agent_pending_reviews 1 -- N agent_evidence_links
strategy_memories 1 -- N agent_evidence_links
