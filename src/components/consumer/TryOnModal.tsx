    ...
    setTimeout(async () => {
      try {
        ...
        if (data.style && data.style.image_url) {
          const tryOnImg = resolveImageUrl(data.style.image_url);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <div className="bg-white p-6 max-w-xl w-full mx-auto rounded-card relative max-h-[90vh] overflow-y-auto custom-scrollbar shadow-lg">
        ...
        {jobStatus === 'idle' && (
          <div>
            <h2 className="text-xl font-bold text-ink mb-2 text-center">{t('title')}</h2>
            <p className="text-ink-second text-xs text-center mb-6">
              {t.rich('choiceSub', {
                styleName,
            ...
            <div className="space-y-4 mb-6">
              ...
              >
                <div>
                  <h3 className="font-semibold text-sm text-ink group-hover:text-primary transition-colors">{t('demoBtnTitle')}</h3>
                  <p className="text-xs text-ink-second mt-1 leading-relaxed">{t('demoBtnDesc')}</p>
                </div>
              ...
              {hasSession ? (
                ...
                >
                  <div>
                    <h3 className="font-semibold text-sm text-ink group-hover:text-primary transition-colors">{t('customBtnTitleActive')}</h3>
                    <p className="text-xs text-ink-second mt-1 leading-relaxed">{t('customBtnDescActive')}</p>
                  </div>
                  <ArrowRight className="w-5 h-5 text-ink-light group-hover:text-primary group-hover:translate-x-1 transition-all" />
                </button>
              ) : (
                <div className="p-4 border border-dashed border-c-border rounded-xl bg-surface-warm/50 flex flex-col md:flex-row items-center justify-between gap-4">
                  <div>
                    <h3 className="font-semibold text-sm text-ink-second">{t('customBtnTitleInactive')}</h3>
                    <p className="text-xs text-ink-light mt-1 leading-relaxed">{t('customBtnDescInactive')}</p>
                  </div>
                  <Link href={`/hand?fromStyleId=${styleId}`} onClick={() => onOpenChange(false)} className="shrink-0">
                    <Button variant="outline" size="sm" className="gap-1 bg-white text-xs">
                      <Camera className="w-3.5 h-3.5" /> {t('scanBtn')}
                    </Button>
                  </Link>
                </div>
              )}
        ...
        {jobStatus === 'failed' && (
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-bold text-ink mb-2">{t('failedTitle')}</h3>
        ...
        {jobStatus === 'success' && resultImage && originalHandImage && (
          <div>
            <h2 className="text-xl font-bold text-ink mb-4 text-center">
              {tryOnMode === 'demo' ? t('successDemoTitle', { styleName }) : t('successTitle')}
            </h2>
            ...
            <div className="relative w-full aspect-[3/4] bg-surface-warm rounded-lg overflow-hidden select-none mb-6 border border-c-border">
              ...
              <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-sm text-white text-[10px] px-2.5 py-1 rounded">{t('before')}</div>
              <div className="absolute bottom-4 right-4 bg-black/60 backdrop-blur-sm text-white text-[10px] px-2.5 py-1 rounded">{t('after')}</div>
            ...
            {recommendations.length > 0 && (
              <div className="mb-6">
                <h4 className="text-[10px] font-bold text-ink-light tracking-widest uppercase mb-3 text-left">{t('recommendTitle')}</h4>
                <div className="grid grid-cols-3 gap-3">
                  {recommendations.map((rec) => {
                    const recImg = resolveImageUrl(rec.image_url);
                    ...
                    try {
                      ...
                      if (colors.length > 0) {
                        ...
                        recName = getTranslatedColor(colors[0]);
                      }
                    } catch (e) {}
